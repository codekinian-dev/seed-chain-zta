/**
 * =============================================================================
 * THROUGHPUT CEILING TEST — SCENARIO 2: HYPERLEDGER + IPFS
 * =============================================================================
 *
 * Arsitektur: Keycloak Auth → IPFS Cluster → Hyperledger Fabric
 * Endpoint:   POST /api/seed-batches (multipart + PDF upload)
...
export default function () {
    ...
    const response = http.post(
        ...
        formPayload,
        { headers: { 'Authorization': `Bearer ${token}` }, tags: { name: 'Throughput2_HL_IPFS' } }
    );
    ...
    if (response.status === 200 || response.status === 201) {
        ...
        errorRate.add(1);
        if (__ITER % 10 === 0) console.error(`[TP2] ✗ ${response.status}`);
    }
}

export function setup() {
    console.log(`=== TP-2 HL+IPFS | Rate=${TARGET_RATE} req/s | Duration=3m ===`);
}
...
export function handleSummary(data) {
    ...
    const csv = `Scenario,Target_Rate,Actual_RPS,Avg_Resp_Time_ms,P95_Resp_Time_ms,P99_Resp_Time_ms,Min_Resp_Time_ms,Max_Resp_Time_ms,Error_Rate,Total_Requests,Test_Duration_sec\nTP_HL_IPFS,${TARGET_RATE},${throughput.toFixed(4)},${(r.values?.avg||0).toFixed(2)},${(r.values?.['p(95)']||0).toFixed(2)},${(r.values?.['p(99)']||0).toFixed(2)},${(r.values?.min||0).toFixed(2)},${(r.values?.max||0).toFixed(2)},${(data.metrics.errors?.values?.rate||0).toFixed(4)},${h.values?.count||0},${d.toFixed(2)}`;

    return {
        [`reports/tp-2-hl-ipfs-rate-${TARGET_RATE}-${ts}.csv`]: csv,
