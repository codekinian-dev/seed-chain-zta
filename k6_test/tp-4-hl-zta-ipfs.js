/**
 * =============================================================================
 * THROUGHPUT CEILING TEST — SCENARIO 4: HYPERLEDGER + ZTA + IPFS (FULL)
 * =============================================================================
 *
 * Arsitektur: Keycloak Auth → ZTA Policy Engine → IPFS Cluster → HL Fabric
 * Endpoint:   POST /api/seed-batches (multipart + PDF upload + ZTA)
...
function getSeedBatchData(iteration) {
    ...
    return {
        ...
        declaredQuantity: String(d.declaredQuantity),
        qtyBaseUnit: 'GRAM',
    };
}
...
export default function () {
    ...
    const formPayload = { ...seedData, document: pdfFile };

    const response = http.post(
        `${API_BASE_URL}/api/seed-batches`,
        formPayload,
        { headers: { 'Authorization': `Bearer ${token}` }, tags: { name: 'Throughput4_HL_ZTA_IPFS' } }
    );
    ...
    if (response.status === 200 || response.status === 201) {
        ...
        errorRate.add(1);
        if (__ITER % 10 === 0) console.error(`[TP4] ✗ ${response.status}`);
    }
}

export function setup() {
    console.log(`=== TP-4 HL+ZTA+IPFS (FULL) | Rate=${TARGET_RATE} req/s | Duration=3m ===`);
}
...
export function handleSummary(data) {
    ...
    const csv = `Scenario,Target_Rate,Actual_RPS,Avg_Resp_Time_ms,P95_Resp_Time_ms,P99_Resp_Time_ms,Min_Resp_Time_ms,Max_Resp_Time_ms,Error_Rate,Total_Requests,Test_Duration_sec\nTP_HL_ZTA_IPFS,${TARGET_RATE},${throughput.toFixed(4)},${(r.values?.avg||0).toFixed(2)},${(r.values?.['p(95)']||0).toFixed(2)},${(r.values?.['p(99)']||0).toFixed(2)},${(r.values?.min||0).toFixed(2)},${(r.values?.max||0).toFixed(2)},${(data.metrics.errors?.values?.rate||0).toFixed(4)},${h.values?.count||0},${d.toFixed(2)}`;

    return {
        [`reports/tp-4-hl-zta-ipfs-rate-${TARGET_RATE}-${ts}.csv`]: csv,
