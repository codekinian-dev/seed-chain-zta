...
class QuerySeedBatchWorkload extends WorkloadModuleBase {
    ...
    async initializeWorkloadModule(workerIndex, totalWorkers, roundIndex, roundArguments, sutAdapter, sutContext) {
        ...
        if (this.batchIds.length === 0) {
            console.error('[QuerySeedBatch-50] ⚠️  NO BATCHES FOUND IN LEDGER!');
            console.error('[QuerySeedBatch-50] Please run Scenario A (createSeedBatch) first to populate data.');
            // Use a placeholder - queries will fail but won't crash
            this.batchIds.push('BATCH-NO-DATA-FOUND');
        }

        console.log(`[Worker ${workerIndex}] Total ${this.batchIds.length} batch IDs for 50 TPS query round`);
