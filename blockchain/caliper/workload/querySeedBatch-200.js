...
class QuerySeedBatchWorkload extends WorkloadModuleBase {
    ...
    async initializeWorkloadModule(workerIndex, totalWorkers, roundIndex, roundArguments, sutAdapter, sutContext) {
        ...
        if (this.batchIds.length === 0) {
            console.error('[QuerySeedBatch-200] ⚠️  NO BATCHES FOUND IN LEDGER!');
            console.error('[QuerySeedBatch-200] Please run Scenario A (createSeedBatch) first to populate data.');
            this.batchIds.push('BATCH-NO-DATA-FOUND');
        }

        console.log(`[Worker ${workerIndex}] Total ${this.batchIds.length} batch IDs for 200 TPS query round`);
